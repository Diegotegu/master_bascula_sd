...
void setup() {
  ...
  if (debugger) {
    if (basc1.getTareTimeoutFlag()) {
      Serial.println("Basc 1 Tare timeout, check MCU>HX711 no.1 wiring and pin designations");
    }
    if (basc2.getTareTimeoutFlag()) {
      Serial.println("Basc 2 Tare timeout, check MCU>HX711 no.1 wiring and pin designations");
    }
  }
